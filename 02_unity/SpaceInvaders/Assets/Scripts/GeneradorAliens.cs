...
public class GeneradorAliens : MonoBehaviour
{
	...
	public Rigidbody2D prefabAlienRojo;
	public Rigidbody2D prefabAlienAzul;
	public Rigidbody2D prefabAlienMarron;
		...
		for (int i = 0; i < filas; i++) {
			for (int j = 0; j < columnas; j++) {
				if (i == 3) {
					...
					Rigidbody2D alien = (Rigidbody2D)Instantiate (prefabAlienAzul, posicion, transform.rotation);
					...
					// Instanciamos el objeto partiendo del prefab
					Rigidbody2D alien = (Rigidbody2D)Instantiate (prefabAlienRojo, posicion, transform.rotation);
					...
					// Instanciamos el objeto partiendo del prefab
					Rigidbody2D alien = (Rigidbody2D)Instantiate (prefabAlienMarron, posicion, transform.rotation);
